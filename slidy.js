...
var w3c_slidy = {
  ...
  hide_slides: function() {
    ...
    else setTimeout(w3c_slidy.hide_slides, 50);
  },
  ...
  init: function() {
    ...
    if (this.ie7) setTimeout(w3c_slidy.ie_hack, 100);
  ...
  toggle_table_of_contents: function() {
    if (w3c_slidy.is_shown_toc()) w3c_slidy.hide_table_of_contents(true);
    else w3c_slidy.show_table_of_contents();
  },
  ...
  toc_click: function(e) {
    ...
    if (target && target.nodeType == 1) {
      ...
      if (uri) {
        ...
        try {
          if (!w3c_slidy.opera) w3c_slidy.set_focus();
        } catch (e) {}
      }
    }
  ...
  replace_by_non_breaking_space: function(str) {
    for (var i = 0; i < str.length; ++i) str[i] = 160;
  },
...
*/
  ...
  outline_click: function(e) {
    ...
    while (target && target.visible == undefined) target = target.parentNode;
    ...
    if (!rightclick && target.visible != undefined) {
      if (target.foldable) {
        if (target.visible) w3c_slidy.fold(target);
        else w3c_slidy.unfold(target);
      }
  ...
  add_initial_prompt: function() {
    ...
    if (this.keyboardless) p1.innerHTML = "swipe left to move to next slide";
    else
      p1.innerHTML =
        "Space, Right Arrow or swipe left to move to " +
  ...
  patch_anchors: function() {
    ...
    var handler = function(event) {
      ...
      } else w3c_slidy.stop_propagation(event);
  ...
  show_slide_number: function() {
    ...
    w3c_slidy.slide_number_element.innerHTML =
      timer +
  ...
  check_location: function() {
    ...
    if (w3c_slidy.slide_number > 0 && (hash == "" || hash == "#"))
      ...
      if (!isNaN(num)) w3c_slidy.goto_slide(num - 1);
    }

    if (w3c_slidy.time_left && w3c_slidy.slide_number > 0) {
      w3c_slidy.show_slide_number();

      if (w3c_slidy.time_left > 0) w3c_slidy.time_left -= 200;
    }
  },
  ...
  set_location: function() {
    ...
    if (w3c_slidy.slide_number >= 0) uri = uri + hash;

    if (typeof history.pushState != "undefined") {
      document.title =
        w3c_slidy.title + " (" + (w3c_slidy.slide_number + 1) + ")";
    ...
    if (this.khtml) hash = "(" + (w3c_slidy.slide_number + 1) + ")";
  ...
  previous_slide: function(incremental) {
    if (!w3c_slidy.view_all) {
      ...
      if (!w3c_slidy.ns_pos) w3c_slidy.refresh_toolbar(200);
    }
  },
  ...
  next_slide: function(incremental) {
    if (!w3c_slidy.view_all) {
      ...
      if (
        ...
        if (last && incremental) w3c_slidy.last_shown = last;
      }
  ...
  set_eos_status: function(state) {
    if (this.eos) this.eos.style.color = state ? "rgb(240,240,240)" : "red";
  },
  ...
  resized: function() {
    ...
    if (width != w3c_slidy.last_width || height != w3c_slidy.last_height) {
      if (width >= 1100) w3c_slidy.size_index = 5;
      // 4
      else if (width >= 1000) w3c_slidy.size_index = 4;
      // 3
      else if (width >= 800) w3c_slidy.size_index = 3;
      // 2
      else if (width >= 600) w3c_slidy.size_index = 2;
      // 1
      else if (width) w3c_slidy.size_index = 0;
  ...
  key_press: function(event) {
    ...
    if (!w3c_slidy.key_wanted) return w3c_slidy.cancel(event);
  ...
  key_down: function(event) {
    ...
    if (!w3c_slidy.slidy_chrome(target) && w3c_slidy.special_element(target))
      return true;
    ...
    if (
      ...
      if (key == 27 || key == 84 || key == 67) return w3c_slidy.cancel(event);
    }

    if (key == 34) {
      // Page Down
      if (w3c_slidy.view_all) return true;
      ...
      if (w3c_slidy.outline) {
        if (w3c_slidy.outline.visible) w3c_slidy.fold(w3c_slidy.outline);
        else w3c_slidy.unfold(w3c_slidy.outline);
      ...
      // F for toggle toolbar
      w3c_slidy.toggle_toolbar();
      return w3c_slidy.cancel(event);
    } else if (key == 65) {
      // A for toggle view single/all slides
      w3c_slidy.toggle_view();
      return w3c_slidy.cancel(event);
    } else if (key == 75) {
      // toggle action of left click for next page
      w3c_slidy.mouse_click_enabled = !w3c_slidy.mouse_click_enabled;
      var alert_msg =
        (w3c_slidy.mouse_click_enabled ? "enabled" : "disabled") +
        " mouse click advance";

      alert(w3c_slidy.localize(alert_msg));
      return w3c_slidy.cancel(event);
    } else if (key == 84 || key == 67) {
      // T or C for table of contents
      if (w3c_slidy.toc) w3c_slidy.toggle_table_of_contents();
  ...
  first_tag: function(element, tag) {
    ...
    if (!this.is_xhtml) tag = tag.toUpperCase();

    for (node = element.firstChild; node; node = node.nextSibling) {
      if (node.nodeType == 1 && node.nodeName == tag) break;
    }
  ...
  mouse_button_click: function(e) {
    ...
    if (
      Math.abs(e.clientX - w3c_slidy.mouse_x) +
        Math.abs(e.clientY - w3c_slidy.mouse_y) >
      10
    )
      return true;

    if (w3c_slidy.selected_text_len > 0) return true;
  ...
  get_key: function(e) {
    ...
    // kludge around NS/IE differences
    if (typeof window.event != "undefined") key = window.event.keyCode;
  ...
  localize: function(src) {
    ...
    var s,
      lookup = w3c_slidy.strings[w3c_slidy.lang];
  ...
  init_localization: function() {
    ...
    if (!w3c_slidy.lang) w3c_slidy.lang = "en";
  }
};
